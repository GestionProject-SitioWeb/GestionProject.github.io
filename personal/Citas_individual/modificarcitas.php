...
if (isset($_SESSION["nombre"])) {
    ...
    if (isset($_GET['id'])) {
        $Id_Maestro = intval($_GET['id']); // Asegúrate de que sea un entero

        // Consultar los datos de la cita
        $sql = "SELECT cm.Id_Maestro, e.nombre AS nombre_estudiante, e.apellido AS apellido_estudiante, 
                   p.nombre AS nombre_personal, p.apellido AS apellido_personal, 
                   cm.motivo, cm.fecha, cm.hora, cm.comentario
            FROM citamaestro cm
            INNER JOIN personales p ON cm.Id_Personal = p.Id_Personal
            INNER JOIN estudiantes e ON cm.Id_Estudiante = e.Id_Estudiante
            WHERE cm.Id_Maestro = ?";
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        $sql_update = "UPDATE citamaestro SET motivo=?, fecha=?, hora=?, comentario=? WHERE Id_Maestro=?";
        $stmt_update = $conexion->prepare($sql_update);
        $stmt_update->bind_param("ssssi", $motivo, $fecha, $hora, $comentario, $Id_Maestro);
            ...
            <div class="py-5 text-center">
                ...
                <form method="POST" action="">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" id="nombre" name="nombre" value="<?php echo htmlspecialchars($nombre_estudiante); ?> <?php echo htmlspecialchars($apellido_estudiante); ?>" readonly>
                    </div>

                    <div class="form-group">
                        <label for="nombre_personal">Nombre Completo del Docente</label>
                        <input type="text" name="nombre_personal" class="form-control" id="nombre_personal" value="<?php echo htmlspecialchars($nombre_personal); ?> <?php echo htmlspecialchars($apellido_personal); ?>" readonly >
                    </div>
